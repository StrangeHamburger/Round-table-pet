...
- **🎵 音乐感知**：读取 Windows 当前媒体播放状态，播放音乐时切换表情、飘音符，并在检测到曲目切换时重新激活动效（由换曲事件触发，而非实时音频振幅分析）
...
> 渲染进程脚本已拆分为 `src/*.js`（零构建，经典 `<script>` + `window.Pet` 命名空间）：

```
main.js          主进程（窗口 / 单实例 / 光标轮询 / 系统托盘 / 开机自启）
├── preload.js   预加载脚本（主进程 ⇄ 渲染进程安全通信）
├── music.ps1    音乐桥接（GSMTC 播放状态）
└── keyboard.ps1 键盘桥接（打字检测）
index.html       渲染进程入口（Canvas 动画 + 设置菜单）
src/
├── config.js    配色 / 尺寸 / 常量配置
├── state.js     运行时状态（位置 / 心情 / 行为 / 持久化）
├── render.js    Canvas 绘制
├── behaviors.js 待机自主行为与互动
├── physics.js   运动 / 拖拽 / 甩飞
├── input.js     鼠标 / 键盘事件
└── app.js       渲染进程启动与主循环
...
- **设置持久化**：配色、尺寸、定家位置、当前位置等偏好会保存，下次启动自动恢复
- **系统托盘**：常驻托盘图标，支持显示 / 隐藏窗口、退出程序
- **开机自启**：可配置登录时自动启动（可选）
- **多显示器漫游**：窗口可在多块显示器之间移动并记住位置

## 🎨 交互设计

- 5 套配色（奶油 / 米白 / 蜜桃 / 薄荷 / 雾蓝），右键菜单一键切换
- 心情系统：正常 / 开心 / 难过 / 惊讶 / 犯困 / 睡觉 / 晕眩，互动触发
- 行为系统：发呆 / 打哈欠 / 伸懒腰 / 打喷嚏 / 打滚 / 躲猫猫 / 钻进鼠标 / 穿屏瞬移 / 高空坠落，随机自主执行；用力往下砸地会扎地沉下、再从屏幕上方落下
